...
df = df_prepro(file_path)

st.title('Sales & Traffic Funnel V1')

options = df.parent_asin.unique().tolist()
selected_option = st.selectbox('Select ASIN:', options, key='funnel')

date_range = df[df.parent_asin == selected_option]['date'].sort_values(ascending=False).dt.strftime('%B-%Y').unique().tolist()
date_selected = st.selectbox('Select Date Range:', date_range)

df_selected = df[(df.parent_asin==selected_option) & (df.month_year == date_selected)]
...
if agree:
    ...
    with col1:
        container = st.container(border=True)
        options_col1 = df.parent_asin.unique().tolist()
        comp_selected_option1 = container.selectbox('Select ASIN to Compare:', options_col1, key='asin1')
        
        date_range = df[df.parent_asin == comp_selected_option1]['date'].dt.strftime('%B-%Y').unique().tolist()
        comp_date_selected1 = container.selectbox('Select Date Range:', date_range, key='date1')
        
        df_selected_col1 = df[(df.parent_asin==comp_selected_option1) & (df.month_year == comp_date_selected1)]
    ...
    with col2:
        container = st.container(border=True)
        options_col2 = df.parent_asin.unique().tolist()
        comp_selected_option2 = container.selectbox('Select ASIN to Compare:', options_col2, key='asin2')
        
        date_range = df[df.parent_asin == comp_selected_option2]['date'].sort_values(ascending=False).dt.strftime('%B-%Y').unique().tolist()
        comp_date_selected2 = container.selectbox('Select Date Range:', date_range, key='date2')
        
        df_selected_col2 = df[(df.parent_asin==comp_selected_option2) & (df.month_year == comp_date_selected2)]
...
trend_asin = st.selectbox('Select ASIN:', options, key='trend2')
vis_df = df[df.parent_asin == trend_asin]
